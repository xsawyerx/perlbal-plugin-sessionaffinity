...
Small note here: this does not mean it will play nice with everything you
already have. Specifically any hooks that rely on the name of the service
(L<Perlbal::Plugin::Addheader>, for example) might be affected. You should
read more below on C<_previous_name>.
...
If you've read the I<Advanced stuff> section above, you might have guessed
a possible problem with anything that relies on the name of the service.

If you're using a plugin that relies on the name of the service (such as
L<Perlbal::Plugin::Addheader>), you will notice it will no longer work
properly. This is because the service that is generated by B<SessionAffinity>
is no longer the previous service, and doesn't contain its name. Instead
it has its own name, which is not known to your plugin.

This is the same approach the B<X-Forwarded-For> header uses.

B<SessionAffinity> tries to help you help the plugin authors fix this. It
adds an unofficial key to the service called C<_previous_name>, which
includes the name of the original service it used. You can check for that
before you check for the service name. if it exists, use it. Otherwise, use
the regular name key.

You might need to unlock the keys of the hash before checking (because Perlbal
was not written in this century and uses L<fields> for some unknown reason.
You can easily unlock the keys using L<Hash::Util>:

    use Hash::Util;
    Hash::Util::unlock_keys( %{$svc} );

    # feel free to inspect $svc->{'_previous_hash'};

In the future, I might consider a really evil hack of changing the name of
the service before sending it off to the hooks, or maybe faking it somehow.
This would be crazy, but if you find a reason for it (like all your plugins
failing), I would be inclined to test it out.
